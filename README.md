...
- **Auto-Increment / Identity Column Support**: Mark database-managed columns with `[JadeDbColumn(IsAutoIncrement = true)]` to automatically exclude them from INSERT and UPDATE statements.
...
Decorate your model with `[JadeDbTable]` to specify the table name (optional — the builder pluralises the class name by default), `[JadeDbColumn]` for any column-name differences, and `[JadeDbColumn(IsAutoIncrement = true)]` for any column whose value is generated by the database (IDENTITY / AUTO_INCREMENT / computed defaults):
...
> **No conventions are assumed.** Unless a property is explicitly decorated with `[JadeDbColumn(IsAutoIncrement = true)]`, it will always be included in INSERT and UPDATE statements. This keeps every schema flexible — models without auto-generated keys work without any special configuration.
...
Properties marked with `[JadeDbColumn(IsAutoIncrement = true)]` are automatically excluded from the generated INSERT statement — you never need to filter them manually.
...
5. **Auto-increment column exclusion** — `BuildInsert` and `BuildUpdate` only exclude a column when it is explicitly decorated with `[JadeDbColumn(IsAutoIncrement = true)]`. There are no name-based conventions, so every schema — including those without a database-generated primary key — works without extra configuration.
